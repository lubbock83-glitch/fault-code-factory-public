-- Powers /diagnostics. Handles four query shapes a tech actually types:
--   "3216"            -> numeric SPN/FMI match
--   "SPN 3216 FMI 4"  -> numeric + full text
--   "DEF derate"      -> symptom keywords
--   "cascadya"        -> misspelling, caught by trigram
-- SECURITY INVOKER (the default) so RLS still hides unapproved drafts.
-- search_path is pinned empty, so every function and operator is schema-qualified.
...
as $$
  ...
  select
    ...
    (
      ...
      + case when p.raw is not null
             then extensions.similarity(fc.name, p.raw) * 0.5 else 0 end
    )::real          as rank,
  ...
  where
    ...
    and (
       ...
       or fc.name operator(extensions.%) p.raw
    )
